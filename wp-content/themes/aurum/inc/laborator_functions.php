...
function laborator_show_pagination( $current_page, $max_num_pages, $from, $to, $pagination_position = 'full', $numbers_to_show = 5 ) {
    ...
    <ul class="pagination<?php echo $pagination_position ? " pagination-{$pagination_position}" : ''; ?>">
		...
		<?php if ( $current_page > 1 ): ?>
            <li class="first_page">
                <a href="<?php echo get_pagenum_link( 1 ); ?>"><?php _e( '&laquo; First', 'aurum' ); ?></a></li>
		<?php endif; ?>

		<?php if ( $current_page > 2 ) : ?>
            <li class="first_page">
                <a href="<?php echo get_pagenum_link( $current_page - 1 ); ?>"><?php _e( 'Previous', 'aurum' ); ?></a>
            </li>
		<?php endif; ?>
		...
		for ( $i = $from; $i <= $to; $i ++ ) :
            ...
            <li<?php echo $is_active ? ' class="active"' : ''; ?>>
                <a href="<?php echo $link_to_page; ?>"><?php echo $i; ?></a></li>
		<?php
		endfor;
		...
		<?php if ( $current_page + 1 <= $max_num_pages ) : ?>
            <li class="last_page">
                <a href="<?php echo get_pagenum_link( $current_page + 1 ); ?>"><?php _e( 'Next', 'aurum' ); ?></a></li>
		<?php endif; ?>

		<?php if ( $current_page < $max_num_pages ): ?>
            <li class="last_page">
                <a href="<?php echo get_pagenum_link( $max_num_pages ); ?>"><?php _e( 'Last &raquo;', 'aurum' ); ?></a>
            </li>
		<?php endif; ?>
    </ul>
    <!-- end: pagination -->
...
function laborator_load_font_style() {
	...
	if ( strpos( $secondary_subset, 'fonts.googleapis.com' ) ) {
		$secondary_font .= '&subset=' . $secondary_subset;
	}

	if ( $primary_font && apply_filters( 'aurum_primary_font_include', true ) ) {
		wp_enqueue_style( 'primary-font', strstr( $primary_font, '://' ) ? $primary_font : ( $api_url . $primary_font ) );
	}

	if ( $secondary_font && $primary_font !== $secondary_font && apply_filters( 'aurum_heading_font_include', true ) ) {
...
function share_story_network_link( $network, $id, $simptips = true ) {
	...
	$icon_class    = 'fa ' . $network_entry['icon'];
	?>
...
function has_transparent_header() {
	return aurum_get_field( 'enable_transparent_header', get_queried_object_id() );
}
...
function laborator_svg_get_size( $image_path ) {
    $svg_size = laborator_svg_get_size_fallback( $image_path );

    if ( $svg_size['width'] && $svg_size['height'] ) {
        return [ $svg_size['width'], $svg_size['height'] ];
    }
...
/**
 * Get SVG width height fallback.
 */
function laborator_svg_get_size_fallback( $image_path ) {
    $svg_content = @file_get_contents( $image_path );

    // Match width and height attributes
    ...
    if ( isset( $width_matches[1] ) && isset( $height_matches[1] ) ) {
        ...
        preg_match( '/viewBox="([^"]+)"/', $svg_content, $view_box_matches );

        if ( isset( $view_box_matches[1] ) ) {
            $viewBox = explode( ' ', $view_box_matches[1] );
            ...
            return false; // No size information found
        }
    }
...
/**
 * Default Value Set for WPBakery Page Builder Loop Parameter Type
 */
...
if ( ! function_exists( 'aurum_show_top_menu_widget' ) ) {
	...
	function aurum_show_top_menu_widget( $widget_id, $is_text_widget = false ) {
		...
		if ( $is_text_widget ) {
			...
			$cart_totals = is_object( WC()->cart ) ? WC()->cart->get_cart_total() : wc_price( 0.0 );
			$widget = sprintf( '<a href="%1$s" class="top-menu--cart-totals">%2$s <span class="top-menu--number">%3$s</span></a>', wc_get_cart_url(), __( 'Cart totals:', 'aurum' ), $cart_totals );
		} // Breadcrumbs
...
/**
 * Reset WooCommerce thumbnail sizes.
 */
function aurum_woocommerce_reset_thumbnail_sizes() {
