...
define( 'LAB_1CL_DEMO_INSTALLER_STYLESHEET', get_template_directory_uri() . '/inc/lib/laborator/laborator-demo-content-importer/demo-content-importer.css' );
...
function lab_1cl_demo_installer_get_packs() {
	return array(
		array(
			# Pack Info
			'name'           => 'Fashion Store',
			'desc'           => 'This will install Aurum fashion store demo content. It includes all theme features.',

			# Pack Data
			'thumb'          => 'demo-content/fashion-store/screenshot.jpg',
			'file'           => 'demo-content/fashion-store/content.xml',
			'options'        => 'demo-content/fashion-store/options.json',
			'layerslider'    => 'demo-content/fashion-store/layerslider-all-sliders.zip',
			'custom_css'     => '',
			...
			'url'			 => 'https://themes.laborator.co/aurum/fashion/'
		),
		...
		array(
			...
			# Pack Data
			'thumb'          => 'demo-content/jewelry-store/screenshot.jpg',
			...
			'url'			 => 'https://themes.laborator.co/aurum/jewelry'
		),
		...
		array(
			...
			# Pack Data
			'thumb'          => 'demo-content/tech-store/screenshot.jpg',
			...
			'url'			 => 'https://themes.laborator.co/aurum/tech/'
		),
		array(
			# Pack Info
			'name'           => 'Bookshop',
			...
			# Pack Data
			'thumb'          => 'demo-content/book-store/screenshot.jpg',
			...
			'url'			 => 'https://themes.laborator.co/aurum/bookshop/'
		),
		array(
			# Pack Info
			...
			# Pack Data
			'thumb'          => 'demo-content/multilingual-site/screenshot.jpg',
			...
			'url'			 => 'https://themes.laborator.co/aurum/multilingual/'
		),
	);
}
...
function lab_1cl_demo_installer_page() {
	...
	$lab_demo_content_url  = get_template_directory_uri() . '/inc/lib/laborator/laborator-demo-content-importer/';
	...
	if ( isset($pack['file']) && $pack['file'])
	{
		...
		if ( ! isset($file_content_pack['disabled']))
		{
			$content_packs[] = array(
				...
				'title'          => 'Media Files',
				'description'    => 'This will download all media files presented in this demo content pack.',
	...
	{		
		...
		case "xml-products":
			...
			if ( $source_details['type'] == 'xml-products' ) {
				$xml_files = is_array( $pack['products'] ) ? $pack['products'] : explode( ',', $pack['products'] );
			} else {
				$xml_files = is_array( $pack['file'] ) ? $pack['file'] : explode( ',', $pack['file'] );
			}
							
			foreach ( $xml_files as $i => $file ) {
				$xml_files[ $i ] = $full_path . $file;
			}

			try {			
				@set_time_limit(0);
				
				foreach ( $xml_files as $i => $xml_file ) {
					...
					$resp[ basename( $xml_file ) ] = $wp_importer;
				}
		...
		case "layerslider":
			...
			try
			{
				...
				{
					$layersliders = is_array( $pack['layerslider'] ) ? $pack['layerslider'] : explode( ',', $pack['layerslider'] );
				
					include LS_ROOT_PATH . '/classes/class.ls.importutil.php';
					
					foreach ( $layersliders as $layerslider ) {
						$ls_file = dirname(__FILE__) . DIRECTORY_SEPARATOR . $layerslider;
						$import = new LS_ImportUtil($ls_file, basename($layerslider));
					}
