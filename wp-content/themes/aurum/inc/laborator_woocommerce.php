...
function aurum_woocommerce_archive_wrapper_start() {
	...
	if ( get_data( 'shop_loop_masonry' ) ) {
		// Enqueue isotope
		wp_enqueue_script( 'aurum-isotope' );
...
if ( ! function_exists( 'aurum_shop_loop_item_title' ) ) {
	...
	function aurum_shop_loop_item_title() {
		...
		$id          = $product->get_ID();
		$link_target = apply_filters( 'aurum_product_link_target', '_self' );
...
if ( ! function_exists( 'aurum_woocommerce_show_cart_title' ) ) {
	...
	function aurum_woocommerce_show_cart_title() {
		...
		?>
        <div class="page-heading woocommerce-page-title<?php echo wc_coupons_enabled() ? ' columns-2' : ''; ?>">
			...
			<?php
			if ( apply_filters( 'aurum_woocommerce_show_coupons_form', wc_coupons_enabled() ) ) {
				?>
...
if ( ! function_exists( 'aurum_woocommerce_product_get_rating_html' ) ) {
	...
	function aurum_woocommerce_product_get_rating_html( $html, $average, $count ) {
        ...
        <div class="star-rating-icons" class="tooltip" data-toggle="tooltip" data-placement="<?php echo ! is_rtl() ? 'right' : 'left'; ?>" title="<?php echo esc_html( $average ); ?> <?php _e( 'out of 5', 'aurum' ); ?>">
			<?php for ( $i = 1; $i <= 5; $i ++ ) : ?>
                <i class="star-icon<?php echo $average >= $i || ( $average > 0 && intval( $average ) == $i - 1 && ( $average - intval( $average ) > 0.49 ) ) ? ' filled' : ''; ?>"></i>
			<?php endfor; ?>
        </div>
		<?php
	...
	function aurum_woocommerce_single_product_wrapper_end() {
		...
		if ( aurum_woocommerce_single_product_get_sidebar_position() ) :
            ...
            <div class="single-product--sidebar">
				...
				<?php
				// Show widgets
				$sidebar = is_active_sidebar( 'shop_single_sidebar' ) ? 'shop_single_sidebar' : 'shop_sidebar';
...
function aurum_woocommerce_before_customer_login_form_title() {
	...
	aurum_woocommerce_section_title( __( 'My account', 'woocommerce' ), __( 'Manage your account and see your orders', 'aurum' ) );
}
 ...
 * Cart Empty
 */
if ( ! function_exists( 'aurum_woocommerce_cart_is_empty' ) ) {

	function aurum_woocommerce_cart_is_empty() {

		?>
        <div class="shop-empty-cart-page wc-empty-cart-message cart-empty">
            ...
            <div class="container">
                <div class="col-sm-12 text-center">
                    ...
                    <div class="cart-empty-title">
                        <h1><?php aurum_woocmmerce_get_translated_string( 'Cart Empty', true ); ?></h1>

						<?php if ( wc_get_page_id( 'shop' ) > 0 ) : ?>
                            <p class="return-to-shop">
                                <a class="button wc-backward" href="<?php echo esc_url( apply_filters( 'woocommerce_return_to_shop_redirect', wc_get_page_permalink( 'shop' ) ) ); ?>">
									<?php aurum_woocmmerce_get_translated_string( 'Browse our products &amp; fill the cart!', true ); ?>
                                </a>
                            </p>
						<?php endif; ?>
                    </div>
...
add_action( 'woocommerce_cart_is_empty', 'aurum_woocommerce_cart_is_empty' );

/**
 * Lost password wrapper start.
 */
if ( ! function_exists( 'aurum_woocommerce_before_lost_password_form_wrapper' ) ) {

	function aurum_woocommerce_before_lost_password_form_wrapper() {
		?>
        <div class="row">
        <div class="col-sm-6">
        <div class="bordered-block woocommerce-ResetPassword-wrapper">
        <h2><?php aurum_woocmmerce_get_translated_string( 'Forgot Password', true ); ?></h2>
		<?php

	}
}

add_action( 'woocommerce_before_lost_password_form', 'aurum_woocommerce_before_lost_password_form_wrapper' );

/**
 * Lost password wrapper end.
 */
if ( ! function_exists( 'aurum_woocommerce_after_lost_password_form_wrapper' ) ) {

	function aurum_woocommerce_after_lost_password_form_wrapper() {
		?>
        </div>
        </div>
        </div>
...
add_action( 'woocommerce_after_lost_password_form', 'aurum_woocommerce_after_lost_password_form_wrapper' );

/**
 * Reset password form wrapper.
 */
function aurum_woocommerce_reset_password_form_start() {
	?>
    <div class="row">
    <div class="col-sm-6">
    <div class="bordered-block woocommerce-ResetPassword-wrapper">
...
add_action( 'woocommerce_before_reset_password_form', 'aurum_woocommerce_reset_password_form_start' );

/**
 * Reset password form wrapper end.
 */
function aurum_woocommerce_reset_password_form_end() {
	?>
    </div>
    </div>
    </div>
...
add_action( 'woocommerce_after_reset_password_form', 'aurum_woocommerce_reset_password_form_end' );


/**
 * Ordering dropdown for products loop
 */
if ( ! function_exists( 'aurum_woocommerce_shop_loop_ordering_dropdown' ) ) {

	function aurum_woocommerce_shop_loop_ordering_dropdown( $catalog_orderby_options, $orderby ) {
		$selected = '';
...
add_action( 'aurum_woocommerce_shop_loop_ordering', 'aurum_woocommerce_shop_loop_ordering_dropdown', 10, 2 );

/**
 * Product categories in single page.
 */
if ( ! function_exists( 'aurum_woocommerce_template_single_categories' ) ) {

	function aurum_woocommerce_template_single_categories() {
		if ( get_data( 'shop_single_product_category' ) ) {
			printf( '<small class="product-terms">%s</small>', get_the_term_list( get_the_id(), 'product_cat', '', ', ' ) );
		}
	}
}

add_action( 'woocommerce_single_product_summary', 'aurum_woocommerce_template_single_categories', 6 );
