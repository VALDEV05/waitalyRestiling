...
$map_style       = aurum_get_field( 'map_style' );

// Decode JSON for the map
if ( ! empty( $map_style ) ) {
	$map_style = json_decode( $map_style );

	if ( ! is_array( $map_style ) ) {
		$map_style = '';
	}
}
...
if ( $show_map ) {
	...
	wp_enqueue_script( 'aurum-google-maps' );
    ...
    <script>
        ...
        <?php if ( $map_style ) : ?>
        var mapStyle = <?php echo json_encode( $map_style ); ?>;
        <?php endif; ?>
    </script>
    <div id="map"></div>
	<?php
    ...
    <div class="container contact-page">
        <div class="row">
                ...
                <div class="contact-form">
                    <div class="row">
                        <div class="col-lg-6">
                            <div class="form-group">
                                <input type="text" name="name" placeholder="<?php _e( 'Name', 'aurum' );
								echo $is_required_name ? ' *' : ''; ?>" class="form-control<?php echo $is_required_name ? ' required' : ''; ?>">
                            </div>
                        </div>
                        <div class="col-lg-6">
                            <div class="form-group">
                                <input type="text" name="subject" placeholder="<?php _e( 'Subject', 'aurum' );
								echo $is_required_subject ? ' *' : ''; ?>" class="form-control<?php echo $is_required_subject ? ' required' : ''; ?>">
                            </div>
                    ...
                    <div class="row">
                        <div class="col-lg-<?php echo $phone_number_field ? 6 : 12; ?>">
                            <div class="form-group">
                                <input type="email" name="email" placeholder="<?php _e( 'E-mail', 'aurum' );
								echo $is_required_email ? ' *' : ''; ?>" class="form-control<?php echo $is_required_email ? ' required' : ''; ?>">
                            </div>
                        </div>
						<?php if ( $phone_number_field ) : ?>
                            <div class="col-lg-6">
                                <div class="form-group">
                                    <input type="text" name="phone" placeholder="<?php _e( 'Phone', 'aurum' );
									echo $is_required_phone ? ' *' : ''; ?>" class="form-control<?php echo $is_required_phone ? ' required' : ''; ?>">
                                </div>
                            </div>
						<?php endif; ?>
                    </div>
                    ...
                    <div class="form-group">
                        <textarea name="message" placeholder="<?php _e( 'Message', 'aurum' );
                        echo $is_required_message ? ' *' : ''; ?>" class="form-control<?php echo $is_required_message ? ' required' : ''; ?>" rows="5"></textarea>
                    </div>
            ...
            <div class="col-lg-5 col-md-5 col-sm-6 contact-information">
				...
				<?php echo do_shortcode( $address_description ); ?>
