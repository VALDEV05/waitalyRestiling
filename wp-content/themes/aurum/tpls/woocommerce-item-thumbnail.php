...
if ( has_post_thumbnail() ) {
	...
	// GIF as featured images
	if ( apply_filters( 'aurum_woocommerce_loop_gif_thumbnails', true ) ) {
		$file_type = wp_check_filetype( wp_get_attachment_url( $post_thumb_id ) );

		if ( 'GIF' === strtoupper( $file_type['ext'] ) ) {
			$thumbnail_size = 'original';
		}
	}

	echo aurum_get_attachment_image( $post_thumb_id, $thumbnail_size );
} else {
